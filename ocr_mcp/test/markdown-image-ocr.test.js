...
test("extractMarkdownImageText continues after one image failure", async () => {
  ...
  const results = await extractMarkdownImageText(markdownPath, { lineNumbers: [1, 3] }, {
    ocrImageText: async (imgPath) => `OCR:${path.basename(imgPath)}`,
  });

  assert.equal(results.length, 2);
  assert.equal(results[0].status, "loaded");
  assert.equal(results[0].text, "OCR:example.png");
